...
function initArray() {
 ...
 function change_color() {
 	...
 	// pos++ is saying pos = pos + 1... so we are incrementing through our array of color indices
 	index++;
 	// If the index in the array is less or greater than our array parameters, we catch this and set it back to 0, to prevent an error from occurring
 	if (index < 0 || index > 13) {
 		index = 0;
 	}
 	...
 	// setTimeout defines the delay between the bg colors being invoked
 	loopID = setTimeout("change_color()",200);
 	// 1000 = 1000 ms = 1 sec
 }
